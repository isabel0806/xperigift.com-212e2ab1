...
function DashboardLayout() {
  ...
  const navItems = [
    ...
    { to: '/dashboard/sales', label: 'Sales', icon: BarChart3 },
    { to: '/dashboard/customers', label: 'Customers', icon: Users },
    { to: '/dashboard/emails', label: 'Emails', icon: Mail },
  ] as const;

  const adminItems = [
    { to: '/dashboard/admin/clients', label: 'Clients', icon: Building2 },
    { to: '/dashboard/admin/bookings', label: 'Audit bookings', icon: Settings },
  ] as const;
    ...
    <div className="flex min-h-screen bg-paper-soft">
      ...
      <aside className="hidden md:flex w-64 shrink-0 flex-col border-r border-hairline bg-paper">
        ...
        <nav className="flex-1 px-3 py-4">
          ...
          {isAdmin && (
            <>
              <p className="mt-6 mb-2 px-3 text-[11px] uppercase tracking-[0.14em] text-ink-muted">Admin</p>
              <ul className="space-y-1">
                {adminItems.map((item) => (
                  <li key={item.to}>
                    <Link
      ...
      <main className="flex-1 pt-14 md:pt-0">
        <Outlet />
        <DashboardContext.Provider value={{ activeClientId, activeClient, isAdmin }}>
          {/* placeholder so context is exported */}
...
interface DashboardContextValue {
  ...
  activeClient: { id: string; name: string; industry: string | null; is_active: boolean } | null;
  isAdmin: boolean;
}
...
export function useDashboard() {
  // Read from localStorage as fallback (works because layout sets it)
  if (typeof window === 'undefined') {
    return { activeClientId: null, activeClient: null, isAdmin: false };
  }
  const id = localStorage.getItem('xg.activeClientId');
  return { activeClientId: id, activeClient: null, isAdmin: false };
}
